...
#define PN532_UART_DRIVER_NAME "pn532_emdev"
#define LOG_GROUP    NFC_LOG_GROUP_COM
#define LOG_CATEGORY "libnfc.emdev"

// Prototypes
int     pn532_ack(nfc_device *pnd);
int     pn532_wakeup(nfc_device *pnd);

#define DRIVER_DATA(pnd) ((struct pn532_uart_data*)(pnd->driver_data))

#if 0
static size_t
pn532_uart_scan(nfc_device *pnd)
{
...
//  驱动程序初始化
  ...
  pnd->emdev  = emdev;
  strcpy(pnd->name, PN532_UART_DRIVER_NAME);
  
  //初始化chip_data
  // Alloc and init chip's data
  if (pn53x_data_new(pnd, &pn532_io) == NULL) {
    perror("malloc");
    emdev->close();
    nfc_device_free(pnd);
    return NULL;
...
pn532_wakeup(nfc_device *pnd)
{
  ...
  int res = pnd->emdev->send(pn532_wakeup_preamble, sizeof(pn532_wakeup_preamble), 0);
...
static int
pn532_send(nfc_device *pnd, const uint8_t *pbtData, const size_t szData, int timeout)
{
  int res = 0;
...
//  uart_flush_input(DRIVER_DATA(pnd)->port, false);
  ...
  res = pnd->emdev->send(abtFrame, szFrame, timeout);
  if (res != 0) {
  ...
  res = pnd->emdev->receive(abtRxBuf, sizeof(abtRxBuf), timeout);
  if (res != 0) {
...
static int
pn532_receive(nfc_device *pnd, uint8_t *pbtData, const size_t szDataLen, int timeout)
{
  uint8_t  abtRxBuf[5];
  size_t len;
  ...
  pnd->last_error = pnd->emdev->receive(abtRxBuf, 5, timeout);
...
//  const uint8_t pn53x_preamble[3] = { 0x00, 0x00, 0xff };
  ...
  // TFI + PD0 (CC+1)
  pnd->last_error = pnd->emdev->receive(abtRxBuf, 2, timeout);
  if (pnd->last_error != 0) {
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR, "%s", "Unable to receive data. (RX)");
    goto error;
  }
  ...
  if (len) {
    pnd->last_error = pnd->emdev->receive(pbtData, len, timeout);
    if (pnd->last_error != 0) {
      log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR, "%s", "Unable to receive data. (RX)");
      goto error;
    }
...
pn532_ack(nfc_device *pnd)
{
  ...
  return (pnd->emdev->send(pn53x_ack_frame, sizeof(pn53x_ack_frame),  0));
}

static int
pn532_abort_command(nfc_device *pnd)
{
  pnd->emdev->close();
  return NFC_SUCCESS;
}

const struct pn53x_io pn532_io = {
  .send       = pn532_send,
  .receive    = pn532_receive,
};

const struct nfc_driver pn532_driver = {
  .name                             = PN532_UART_DRIVER_NAME,
  .scan_type                        = INTRUSIVE,
  .scan                             = NULL,//pn532_uart_scan,
  .open                             = pn532_open,
  .close                            = pn532_close,
  ...
  .abort_command  = pn532_abort_command,
  .idle           = pn53x_idle,
